...
function renderMessageBody(m){
  ...
  return linkifyText(renderMentionText(escapeHtml(m.content||'')));
}
...
/*
  يحوّل أي رابط مكتوب داخل نص متمّ عليه escapeHtml بالفعل إلى <a> قابل للضغط.
  بيتعرف على: https://، http://، وwww. من غير بروتوكول.
  لازم يتنفذ بعد escapeHtml عشان النص يبقى آمن، ولازم الـ regex ميلمسش أي <span>
  متضاف من renderMentionText قبله، فبنشتغل على النص كامل بما فيه الـ tags الآمنة دي.
*/
function linkifyText(escapedHtml){
  if(!escapedHtml) return escapedHtml;
  const urlRegex = /(\bhttps?:\/\/[^\s<]+[^\s<.,!?:;'")\]]|(?<![\w@.])www\.[^\s<]+[^\s<.,!?:;'")\]])/gi;
  return escapedHtml.replace(urlRegex, (match)=>{
    const href = match.startsWith('http') ? match : `https://${match}`;
    return `<a href="${href}" target="_blank" rel="noopener noreferrer nofollow" class="msg-link" onclick="event.stopPropagation()">${match}</a>`;
  });
}
...
async function loadMessageMedia(m){
  ...
  const url = await getFileUrlCached(m.telegram_file_id);
  if(!url) return;
  el.innerHTML = `<img src="${url}" loading="lazy" onclick="event.stopPropagation();openImageViewer('${url}')">`;
}
...
function subscribeToRoom(roomId){
  ...
  roomChannel = sb.channel('room-'+roomId)
    ...
    .subscribe((status)=>{
      // لو الاتصال اتقطع (شبكة ضعيفة مثلاً)، نجيب أي رسايل فاتتنا مرة واحدة لما يرجع يشتغل
      if(status === 'SUBSCRIBED'){
        pollRoomMessages(roomId); // مسحة أخيرة لأي حاجة فاتت وقت الانقطاع
      }
    });
}
    ...
    <div class="view">
      <div class="profile-header">
        ...
        <div class="profile-body">
          ...
          <div style="color:var(--ink-dim);font-size:14px">${p.bio?linkifyText(escapeHtml(p.bio)):(isOwn?'مفيش نبذة لسه — اضغط تعديل عشان تضيف واحدة':'')}</div>
...
async function loadGalleryThumb(itemId, fileId, isOwn){
  ...
  holder.innerHTML = `<img class="gallery-thumb" src="${url}" loading="lazy">${isOwn?`<div style="text-align:center;font-size:11px;color:var(--red);cursor:pointer;margin-top:2px" onclick="deleteGalleryItem('${itemId}')">حذف</div>`:''}`;
}
...
async function loadProfileMediaThumb(it){
  ...
  const url = await getFileUrlCached(it.fileId);
  if(!url) return;
  holder.innerHTML = `<img class="gallery-thumb" src="${url}" loading="lazy" onclick="window.open('${url}','_blank')">`;
}
