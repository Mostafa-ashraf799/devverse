...
async function renderBookmarks(container){
  ...
  const { data: marks, error } = await sb.from('bookmarks').select('*').eq('user_id', currentUser.id).order('created_at', {ascending:false});
...
async function openDmConversation(otherId, otherUsername){
  ...
  dmPollInterval = setInterval(()=> pollDmMessages(otherId), 3000);
...
async function pollDmMessages(otherId){
  if(otherId !== activeDmUserId) return;
  refreshDmReadReceipts(otherId);
  const since = lastSeenDmMsgAt[otherId];
  if(!since) return;
  const { data: rows } = await sb.from('chat_messages')
    .select('id, created_at, sender_id, receiver_id, room_id')
    .is('room_id', null)
  ...
  if(rows?.length){
    for(const row of rows){
      const belongsHere = (row.sender_id===currentUser.id && row.receiver_id===otherId) || (row.sender_id===otherId && row.receiver_id===currentUser.id);
      if(belongsHere) await appendDmMessageIfNew(row.id);
    }
  }
}
