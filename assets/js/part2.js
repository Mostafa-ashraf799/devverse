...
function getCachedFile(fileId){
  try{ return localStorage.getItem('dvfile:'+fileId); }catch(e){ return null; }
}
function setCachedFile(fileId, dataUrl){
  try{ localStorage.setItem('dvfile:'+fileId, dataUrl); }catch(e){ /* storage full or unavailable — safe to ignore, just won't cache */ }
}
...
async function loadAvatarInto(elId, fileId){
  ...
  if(cached){ applyAvatarBg(el, cached); return; }
  ...
  try{
    ...
    setCachedFile(fileId, dataUrl);
    applyAvatarBg(el, dataUrl);
  }catch(e){
    applyAvatarBg(el, URL.createObjectURL(blob));
  }
}
...
async function loadPostFile(postId, type, fileId){
  ...
  const cached = getCachedFile(fileId);
  if(cached){
    holder.innerHTML = `<img src="${cached}">`;
    return;
  }
...
async function toggleComments(postId){
  ...
  const { data: comments } = await sb.from('post_comments').select('*, profiles(username)').eq('post_id', postId).eq('is_removed', false).order('created_at');
  box.innerHTML = (comments||[]).map(c=>`<div class="comment"><b class="name" style="cursor:pointer" onclick="viewProfile('${c.author_id}')">@${c.profiles?.username||''}</b> ${escapeHtml(c.content)}</div>`).join('')
...
async function renderArticles(container){
  ...
  try{
    const { data: articles, error } = await sb.from('articles').select('*, profiles!author_id(username, full_name)').eq('is_draft', false).eq('is_removed', false).order('is_pinned', {ascending:false}).order('created_at', {ascending:false});
    if(error){ listEl.innerHTML = `<div class="empty-state" style="color:var(--red)"><div class="icon"><i class="fa-solid fa-triangle-exclamation"></i></div>تعذر تحميل المقالات: ${escapeHtml(error.message)}</div>`; return; }
    if(!articles?.length){ listEl.innerHTML = `<div class="empty-state"><div class="icon"><i class="fa-solid fa-pen-to-square"></i></div>لسه مفيش مقالات منشورة</div>`; return; }
    ...
    listEl.innerHTML = `<div class="empty-state" style="color:var(--red)"><div class="icon"><i class="fa-solid fa-triangle-exclamation"></i></div>حصل خطأ غير متوقع أثناء تحميل المقالات: ${escapeHtml(e.message)}<br><span style="font-size:11px;color:var(--ink-faint)">افتح الـ Console (F12) للتفاصيل الكاملة</span></div>`;
  }
}
...
async function openArticleReader(articleId){
  ...
  try{
    ...
    const { data: comments } = await sb.from('article_comments').select('*, profiles(username)').eq('article_id', articleId).eq('is_removed', false).order('created_at');
    ...
    c.innerHTML = `<div class="view">
      ...
      <div class="post-actions" style="margin-bottom:20px">
        ...
        <div class="act copy-btn" onclick="copyText(${JSON.stringify(a.content_markdown||'')})"><i class="fa-solid fa-copy"></i> نسخ</div>
...
let roomsListPollInterval = null;
let myRoomStatusSnapshot = {};

function startRoomsListPolling(){
  if(roomsListPollInterval) clearInterval(roomsListPollInterval);
  pollMyRooms();
  roomsListPollInterval = setInterval(pollMyRooms, 5000);
}
